        ...
        sections: [
            ...
            <ul>
              ...
              <li><strong>功能实现逻辑：</strong>
                <ul>
                  ...
                  <li>订单详情弹窗：分Tab展示订单信息（Tab0：订单号/状态/时间/渠道/销售员/备注，新增"观演人信息"块展示观演人姓名+手机号，一个订单仅记录一个观演人）、演出座位（Tab1：演出名称/场馆/演出厅/场次/数量及座位明细表格，座位明细表格新增"核销码"列展示门票级核销码）、支付减免（Tab2：支付信息含商品总价/优惠减免/实付金额/支付时间/交易单号/组合支付明细；减免明细表格始终显示5种减免类型行：票品折扣/优惠券/券包/会员折扣/积分抵扣，有数据显示具体名称和金额，无数据显示0）、出票日志（Tab3：展示订单级核销码（O前缀15位）+ 门票级核销码表格（T前缀18位，每张票一个），出票明细折叠面板含取票码、操作日志时间轴）。底部操作按钮：已支付状态显示"确认核销"和"申请退款"；预约状态显示"取消订单"和预约到期时间</li>
                  ...
                  <li>核销码体系：采用两级核销码结构，核销码渠道标识为动态channel_id（2位），关联sales_channels表。订单级核销码（O+YYMMDD+渠道ID(2位)+流水号(6位)，共15位）用于订单维度核销；门票级核销码（T+YYMMDD+渠道ID(2位)+订单标识(6位)+订单内序号(3位)，共18位）每张票一个，用于门票维度核销</li>
                  ...
                  <li>订单状态流转：预约 → 收款 → 已支付 → 出票 → 已出票 → 核销 → 已核销；预约 → 取消订单 → 已取消；预约超时未支付 → 已取消；已支付/已出票 → 退款中 → 已退款</li>
                </ul>
              </li>
          ...
          },
          {
            ...
            <ul>
              ...
              <li><strong>执行中：</strong>弹出订单详情弹窗（720px），分4个Tab展示完整订单数据：Tab0订单信息含"观演人信息"块（展示观演人姓名+脱敏手机号，一个订单仅记录一个观演人）；Tab1演出座位明细表格新增"核销码"列展示门票级核销码（T前缀18位）；Tab2支付减免；Tab3出票日志展示订单级核销码（O前缀15位）+ 门票级核销码表格（T前缀18位，每张票一个）</li>
